...
Yandex Cloud Serverless Functions can securely access sensitive data like API keys, database passwords, and other
credentials using Yandex Lockbox. This feature allows secrets to be managed centrally and securely, and then injected
into the function's environment at runtime as environment variables. This avoids hardcoding secrets in source code,
...
- The solution must be backward-compatible; existing workflows without secrets must continue to work without
  modification.
...
1. **Input Configuration**: The action's configuration in `action.yml` will be extended to accept a multi-line `secrets`
   input.
2. **Parsing**: A dedicated parser will process the `secrets` input string. Each line will represent a single secret and
   its mapping to an environment variable. The parser will be located in `src/parse/lockboxVariables.ts`.
...
5. **Documentation & Testing**: The `README.md` will be updated to document the new feature, and comprehensive unit
   tests will be added for the parsing and version resolution logic.
...
- `environment-variable` (required): The name of the environment variable that the secret's value will be assigned to
  within the function.
- `secret-id` (required): The unique identifier of the secret in Lockbox.
- `version-id` (required): The identifier of the secret's version. The special value `latest` can be used to
...
2. **Implement Parser**:
    ...
    - This function will take an array of strings (from the input) and return an array of structured `Secret` objects
...
3. **Implement 'latest' Version Resolution**:
    ...
    - For each secret with `versionId === 'latest'`, this function will use the `secretService.get` method from the
...
5. **Testing**:
    ...
    - Add unit tests for `resolveLatestLockboxVersions`, using mocks for the Lockbox SDK client to simulate API
      responses.
...
- How should the action behave if a secret ID or version ID (when not `latest`) is not found?
    - _Decision_: The Yandex Cloud API will reject the request. The action should fail with the error message from the
      API. For `latest` resolution, if a secret is not found, the action will fail early with a clear error message.
- Are there other version aliases to consider besides `latest`?
    - _Decision_: For now, only `latest` will be supported as a special alias. Other aliases can be added in the future
      if required.
